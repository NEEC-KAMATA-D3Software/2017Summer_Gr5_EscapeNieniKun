...
namespace Action.Scene
{
    class Haikei
    {
        ...
        public Haikei(GameDevice gameDevice)
        {
        ...
        private List<GameObject> AddBlock(int lineCnt, string[] line)
        {
            ...
            objectList.Add("8", new SmallWood(Vector2.Zero, gameDevice));
            ...
            foreach (var s in line)
            {
                try
                {
                    ...
                    work.SetPositon(new Vector2(colCnt * 128, lineCnt * 128));
                    workList.Add(work);
                }
                catch (Exception e)
                {
        ...
        public void Hit(GameObject gameObject)
        {
            ...
            for (int i = y - 1; i <= (y + 1); i++)
            {
                for (int j = x - 1; j <= (x + 1); j++)
                {
                    ...
                    //空白なら何もしない
                    if ((obj is Space) && (obj is SmallWood))
                    {
                        continue;
                    }
